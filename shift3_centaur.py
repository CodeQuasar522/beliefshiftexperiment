...
# Verify centaur model exists in ollama
print("Checking for centaur model in ollama...")
try:
    ollama.show("centaur")
    print("Centaur model found in ollama")
except Exception:
    print("ERROR: 'centaur' model not found in ollama.")
    print("Run setup_centaur.py first to download, convert, and register the model.")
    sys.exit(1)
...
def evaluate(history):
    """
    Evaluate belief using Centaur-70B via ollama.
    Uses << >> choice format that Centaur is trained on.
    ...
    # Fallback
    print(f"\t\t[Centaur] No match, defaulting to neutral")
    return "neutral"
...
def run_single(statement, type, appeal, framing):
    ...
    # Initial belief evaluation - choices in << >> format for Centaur
    history = [
        system_msg,
        {"role": "user",
    ...
    # Record assistant response in << >> format (consistent with Centaur training)
...
def dtest():
    ...
    print("=== TEST MODE (Centaur-70B): Running on 2 claims ===")
    print(f"Claims: {test_claims}\n")
    start = time.time()
    results = run_all_conditions(test_claims, "opinion", "test_results_centaur.csv")
...
def main():
    ...
    if len(args) < 4:
        print("Usage: python shift3_centaur.py [claim] [type] [appeal] [framing]")
    ...
    if "-all" in args:
        ...
        run_all_conditions(facts, "fact", "fact_results_centaur.csv")
        run_all_conditions(opins, "opinion", "opinion_results_centaur.csv")
        return
